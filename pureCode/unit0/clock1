...
scene.title = "This clock uses radios to change the time. Select a time for each section."
...
for i in range(12):
    global disabledSelect
    selectHr.append(radio(bind=changeHr, text=str(i+1)+":00", id=i+1, name="hrGroup", disabled=False))
    scene.append_to_caption('\n')
...
for i in range(60):
    ...
    selectMin.append(radio(bind=changeMin, text="xx:"+minute, id=i+1, name="minGroup", disabled=False))
